...
ipc_msr_pat <- site_char %>%
  ...
  mutate(
    ...
    variable = factor(variable, levels = rev(ipc_msr_patient_names))
  )
...
ipc_dist_names <- c(
  ...
  "distance_patients" = "Restr. no. of patients",
  "distance_visitors" = "Restr. no. of visitors"
)
...
restr_addr_names <- c(
  ...
  "tb_restr_addr_dot" = "Changes in DOT provision"
)
...
```{r plot}
types <- c(
  "Have you implemented distancing and separation measures?",
  "Have you implemented environmental protective measures?",
  "Have you implemented personal protective measures for staff?",
  "Have you implemented personal protective measures for patients?",
  "Have you observed changes in services during the pandemic?",
  "Have you observed drug shortages during the pandemic?",
  "Have you experienced restrictions in TB services during the pandemic?",
  "How did you address disruptions in TB services?",
  "How did you change DOT provisions to address disruptions in TB services?"
)
...
site_char_main <- site_char %>%
  mutate(
    ...
    site_setting = factor(
      ifelse(site_setting == "urban", "Urban", "Peri-urban/rural"),
      levels = c("Peri-urban/rural", "Urban")
    )
  ) %>%
  dplyr::select(
    record_id,
...
ipc <- rbind(
  ...
  summarise(mean_n_ipc = mean(n_ipc)) %>%
  ungroup()

ipc_assoc_pl <- ipc %>%
  ggplot(aes(x = char, fill = value, y = mean_n_ipc)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~variable, scales = "free_x") +
  scale_fill_manual(values = cols[-c(5:6)]) +
  scale_y_continuous(expand = c(0, 0)) +
  labs(y = "Mean number of implemented IPC measures") +
  theme_custom() +
  theme(
    legend.position = "top",
    legend.direction = "vertical",
...
pl <- ggplot(
  ...
  scale_x_continuous(
    ...
    breaks = seq(2018, 2023, 1) # seq(2018, 2024, 2)
  ) +
  scale_y_continuous(
    expand = c(0, 0),
    labels = scales::comma
